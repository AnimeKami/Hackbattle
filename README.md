...
| `latency_sec` | Time from flash to that minimum (constriction speed proxy). |
| `pct_constriction` | `(baseline - min) / baseline * 100` — how much the pupil shrank. |

All units are pixels, not millimeters — this is a *relative*, session-local
measurement (good for before/after or trend comparisons on the same person
with the same camera), not an absolute clinical measurement. Do not present
`_px` values as calibrated physical sizes.

## Known limitations (be upfront about these, don't paper over them)

- **Not calibrated to real-world units.** Pixel diameter depends on camera
  distance/zoom — only relative changes within one session are meaningful.
