...
async function transmitData() {
  ...
  try {
    const res = await fetch(`/motor/api/forward/${window.selectedMode}`, {
      method: "POST",
...
function renderBackendResults(data) {
  const resultKey = data.finding === "inconclusive"
    ? "inconclusive"
    : data.flag_doctor ? "consult" : "normal";
  const f = data.features || {};

  try {
    const shared = JSON.parse(localStorage.getItem("neuroscreen.latestResults") || "{}");
    shared.session_id = shared.session_id || `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    shared.motor = { ...data, module: "motor", session_id: shared.session_id, saved_at: new Date().toISOString() };
    localStorage.setItem("neuroscreen.latestResults", JSON.stringify(shared));
  } catch (storageError) {
    console.warn("Could not save shared motor result:", storageError);
  }

  if (window.selectedMode === "tremor") {
    MOCK_DATA[resultKey] = {
      flag_doctor: data.flag_doctor,
      finding: data.finding || (resultKey === "inconclusive" ? "inconclusive" : "normal"),
      severity: data.severity || "unknown",
      assessment: data.reasons?.join(" ") || "Analysis complete.",
