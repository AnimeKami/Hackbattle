...
def analyze_frame_bgr(frame_bgr, face_mesh, return_details=False):
    ...
    if has_iris:
        ...
        if d_left is not None and d_right is not None:
            ...
            if return_details:
                return {"diameter": float(d_final), "iris_diameter": measure_iris_diameter_px(landmarks, w, h), "confidence": confidence,
                        "left_diameter": float(d_left), "right_diameter": float(d_right),
                        "left_confidence": float(np.clip(score_left, 0.0, 1.0)), "right_confidence": float(np.clip(score_right, 0.0, 1.0))}
            ...
            if return_details:
                return {"diameter": float(d_left), "iris_diameter": measure_iris_diameter_px(landmarks, w, h), "confidence": float(np.clip(score_left, 0.0, 1.0)),
                        "left_diameter": float(d_left), "right_diameter": None,
                        "left_confidence": float(np.clip(score_left, 0.0, 1.0)), "right_confidence": 0.0}
            return float(d_left)
            ...
            if return_details:
                return {"diameter": float(d_right), "iris_diameter": measure_iris_diameter_px(landmarks, w, h), "confidence": float(np.clip(score_right, 0.0, 1.0)),
                        "left_diameter": None, "right_diameter": float(d_right),
                        "left_confidence": 0.0, "right_confidence": float(np.clip(score_right, 0.0, 1.0))}
            ...
            return {"diameter": None, "iris_diameter": measure_iris_diameter_px(landmarks, w, h), "confidence": 0.0,
                    "left_diameter": None, "right_diameter": None,
                    "left_confidence": 0.0, "right_confidence": 0.0} if return_details else None
    else:
        ...
        result = {
            ...
            "left_diameter": float(d_l) if d_l is not None else None,
            "right_diameter": float(d_r) if d_r is not None else None,
            "left_confidence": float(np.clip(score_l, 0.0, 1.0)) if d_l is not None else 0.0,
            "right_confidence": float(np.clip(score_r, 0.0, 1.0)) if d_r is not None else 0.0,
        }
